...
# ─────────────────────────────────────────────────────────────────────────
# 1. CARGA — idéntica a train_ml_module.py 
# ─────────────────────────────────────────────────────────────────────────
...
def flatten(d, parent_key=""):
    """Aplana un dict/list anidado en pares 'a.b.c': valor, para poder
    buscar claves sin conocer de antemano la estructura exacta del JSON."""
    items = {}
    if isinstance(d, dict):
        ...
        for i, v in enumerate(d):
            items.update(flatten(v, f"{parent_key}[{i}]"))
    else:
        items[parent_key] = d
    return items


def find_auc(flat_dict, model_aliases):
    """Busca en el dict aplanado una clave que contenga 'auc' y alguno de
    los alias del modelo (insensible a mayúsculas/espacios/guiones).
    Prioriza rutas que contengan 'test' sobre las que contengan 'cv'/'train',
    ya que exp05 compara siempre contra el AUC de test held-out."""
    def norm(s):
    ...
    for k, v in flat_dict.items():
        ...
        if "auc" not in nk:
            continue
        if not any(norm(alias) in nk for alias in model_aliases):
            continue
        if not isinstance(v, (int, float)):
            continue
        priority = 0 if "test" in nk else (1 if not any(t in nk for t in ("cv", "train")) else 2)
...
if exp04_results_path.exists():
    ...
    for model_name in ["RandomForest", "XGBoost"]:
        key, auc_exp04 = find_auc(flat_exp04, aliases[model_name])
        auc_exp05 = results["models"][model_name]["test"]["auc_roc"]
        if auc_exp04 is not None:
            found_any = True
            delta = auc_exp05 - auc_exp04
            print(f"{model_name:15s}  exp04 AUC: {auc_exp04:.4f}  (clave: {key})")
            print(f"{'':15s}  exp05 AUC: {auc_exp05:.4f}   Δ = {delta:+.4f}")
        else:
            print(f"{model_name:15s}  exp04 AUC: no encontrado automáticamente   "
                  f"exp05 AUC: {auc_exp05:.4f}")

    if not found_any:
        print("\nNo se pudo localizar el AUC de exp04 automáticamente. "
              "Estructura real de resultados/exp04/ml_results.json "
              "(claves de primer y segundo nivel):")
        for k, v in exp04.items():
            if isinstance(v, dict):
                print(f"  {k}: {list(v.keys())}")
            else:
                print(f"  {k}: {v}")
        print("\nCompara manualmente usando estas claves"
              "(AUC test: RF=0.9595, XGBoost=0.9596).")
else:
    print(f"\n[AVISO] No se encontró {exp04_results_path} — "
          "compara manualmente (AUC test: "
